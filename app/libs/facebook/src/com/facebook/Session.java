...
public class Session implements Serializable {
    ...
    /**
     * The default mddn.swen.headbanger.activity code used for authorization.
     *
    ...
    Session(Context context, String applicationId, TokenCachingStrategy tokenCachingStrategy,
        ...
        // if the mddn.swen.headbanger.application ID passed in is null, try to get it from the
    ...
    /**
     * Returns the mddn.swen.headbanger.application id associated with this Session.
     *
     * @return the mddn.swen.headbanger.application id associated with this Session.
     */
     ...
     * will result in a warning, and may fail during server-side authorization. Also, an mddn.swen.headbanger.application
     * must have at least basic read permissions prior to requesting publish permissions, so
     * this method should only be used if the mddn.swen.headbanger.application knows that the user has already granted
     * read permissions to the mddn.swen.headbanger.application; otherwise, openForRead should be used, followed by a
     ...
     * only in instances where an mddn.swen.headbanger.application has previously obtained an access token and wishes
     * to import it into the Session/TokenCachingStrategy-based session-management system. An
     * example would be an mddn.swen.headbanger.application which previously did not use the Facebook SDK for Android
        ...
        if (request != null && !request.isLegacy) {
            ...
            if (!resolveIntent(intent)) {
                throw new FacebookException(String.format(
                        "Cannot use SessionLoginBehavior %s when %s is not declared as an mddn.swen.headbanger.activity in AndroidManifest.xml",
    ...
    public static final class Builder {
        ...
        /**
         * Sets the mddn.swen.headbanger.application id for the Session.
         *
    ...
    private void autoPublishAsync() {
        ...
        synchronized (this) {
            if (autoPublishAsyncTask == null && Settings.getShouldAutoPublishInstall()) {
                // copy the mddn.swen.headbanger.application id to guarantee thread safety against our container.
                String applicationId = Session.this.applicationId;

                // skip publish if we don't have an mddn.swen.headbanger.application id.
