 ...
 * executed in a batch, a Facebook mddn.swen.headbanger.application ID must be associated with the batch, either by supplying a Session for
...
public class Request {
     ...
     *            for the Facebook user is determined.  If there has been no app ID set, an exception will be thrown.
     * @param applicationId
     *            explicitly specified Facebook App ID.  If null, and there's a valid session, then the mddn.swen.headbanger.application ID
     *            from the session will be used, otherwise the mddn.swen.headbanger.application ID from metadata will be used.
     * @param callback
     *            a callback that will be called when the request is completed to handle success or error conditions.
     *            The GraphObject in the Response will contain a "custom_audience_third_party_id" property that
     *            represents the user as described above.
    ...
    /**
     * Gets the default Facebook mddn.swen.headbanger.application ID that will be used to submit batched requests if none of those requests
     * specifies a Session. Batched requests require an mddn.swen.headbanger.application ID, so either at least one request in a batch must
     * specify a Session or the mddn.swen.headbanger.application ID must be specified explicitly.
     *
     * @return the Facebook mddn.swen.headbanger.application ID to use for batched requests if none can be determined
     */
    ...
    /**
     * Sets the default mddn.swen.headbanger.application ID that will be used to submit batched requests if none of those requests specifies
     * a Session. Batched requests require an mddn.swen.headbanger.application ID, so either at least one request in a batch must specify a
     * Session or the mddn.swen.headbanger.application ID must be specified explicitly.
     *
     * @param applicationId
     *            the Facebook mddn.swen.headbanger.application ID to use for batched requests if none can be determined
     */
    ...
    /**
     * Sets the tag on the request; this is an mddn.swen.headbanger.application-defined object that can be used to distinguish
    ...
    /**
     * Gets the tag on the request; this is an mddn.swen.headbanger.application-defined object that can be used to distinguish
     * between different requests. Its value has no effect on the execution of the request.
     *
    ...
    private void addCommonParameters() {
        if (this.session != null) {
            ...
            if (!Utility.isNullOrEmpty(appID) && !Utility.isNullOrEmpty(clientToken)) {
                ...
                Log.d(TAG,
                        "Warning: Sessionless Request needs token but missing either mddn.swen.headbanger.application ID or client token.");
            }
        }
