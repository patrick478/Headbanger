...
public abstract class PickerFragment<T extends GraphObject> extends Fragment {
    ...
    void layoutActivityCircle() {
        // If we've got no data, make the mddn.swen.headbanger.activity circle full-opacity. Otherwise we'll dim it to avoid
        //  cluttering the UI.
    ...
    void hideActivityCircle() {
        if (activityCircle != null) {
            // We use an animation to dim the mddn.swen.headbanger.activity circle; need to clear this or it will remain visible.
